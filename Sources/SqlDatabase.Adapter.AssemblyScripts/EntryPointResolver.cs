...
internal sealed class EntryPointResolver
{
    ...
    public IEntryPoint? Resolve(Assembly assembly)
    {
        ...
        {
            Log.Error($"Fail to create instance of {type.FullName}.", ex);
            return null;
        }
    ...
    private Type? ResolveClass(Assembly assembly)
    {
        ...
        {
            Log.Error($"public class {ExecutorClassName} not found.");
            return null;
        }

        if (candidates.Count != 1)
        {
            Log.Error($"There are {candidates.Count} items with signature public class {ExecutorClassName}.");
            return null;
        }
    ...
    private ExecuteMethodRef ResolveMethod(Type type)
    {
        ...
        if (methods.Count == 0)
        {
            Log.Error($"public void {ExecutorMethodName}(IDbCommand command, IReadOnlyDictionary<string, string> variables) not found in {type}.");
        }
