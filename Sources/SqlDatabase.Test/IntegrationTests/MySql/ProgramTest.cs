...
namespace SqlDatabase.IntegrationTests.MySql
{
    [TestFixture]
    public class ProgramTest
    {
        private readonly string _connectionString = new MySqlConnectionStringBuilder(MySqlQuery.ConnectionString) { Database = "sqldatabasetest_it" }.ToString();
        ...
        public void BeforeEachTest()
        {
            ...
            _scriptsLocation = ConfigurationManager.AppSettings["MySql.IntegrationTestsScriptsLocation"];
...
FROM person person
     INNER JOIN person_address person_address ON (person_address.person_id = person.id)
ORDER BY person.id";
...
FROM module_a_person p
     LEFT JOIN module_b_person_address a ON a.person_id = p.id
ORDER BY p.name";
        ...
        public void ExportDataToConsole()
        {
            ...
            var args = new GenericCommandLineBuilder()
                ...
                .SetExportToTable("exported_data1")
                .BuildArray();
            ...
            using (var c = new MySqlConnection(_connectionString))
            {
                ...
                var test = c.ExecuteScalar("SELECT COUNT(1) FROM exported_data1");
                test.ShouldBe(2);
            }
        }
        ...
        public void ExportDataToFile()
        {
            using (var output = new TempFile(".sql"))
            {
                ...
                var args = new GenericCommandLineBuilder()
                    ...
                    .SetConnection(_connectionString)
                    .SetScripts(Path.Combine(_scriptsLocation, @"Export\export.sql"))
                    .SetExportToTable("exported_data2")
            ...
            using (var c = new MySqlConnection(_connectionString))
            {
                ...
                var test = c.ExecuteScalar("SELECT COUNT(1) FROM exported_data2");
                test.ShouldBe(2);
            }
        }
        ...
        public void ExecuteScript()
        {
            ...
            var sql = "SELECT 1 FROM information_schema.schemata WHERE LOWER(schema_name) = LOWER('{0}')".FormatWith(new MySqlConnectionStringBuilder(_connectionString).Database);

            using (var c = new MySqlConnection(MySqlQuery.ConnectionString))
            {
                c.Open();
        ...
        private IDatabase CreateDatabaseObject(AppConfiguration configuration = null)
        {
            return new Database
            {
                Adapter = new MySqlDatabaseAdapter(_connectionString, configuration ?? _configuration, new Mock<ILogger>(MockBehavior.Strict).Object)
            };
        }
    }
}
